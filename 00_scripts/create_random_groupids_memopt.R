...
convert_locality_id <- function(x) {
  return(as.integer(substr(x,2,15)))
}
...
if (to_run == TRUE) {
  ...
  locs$timegroups <- match(locs$timegroups, timegroups_names$timegroups)
  locs$group.id <- convert_group_id(locs$group.id)
  locs$LOCALITY.ID <- convert_locality_id(locs$LOCALITY.ID)
  grouped_locs <- locs %>% group_by(LOCALITY.ID, month, timegroups)
  ...
  randomgroupids = foreach(i = 1:1000, .combine = 'cbind') %dopar%
    createrandomlocs_grouped(grouped_locs)

  parallel::stopCluster(cl = my.cluster)
 
  save(randomgroupids, file = write_path)
  message(paste("Generated:",write_path))
